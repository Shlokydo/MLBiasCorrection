...
!  REAL(r_size) :: obserr=0.2d0
  ...
  INTEGER :: idnci, idnco, idde, idvvf, idvva ,idvvfm, idvvam 
...
!          WRITE(92) x4
    ...
    DO n=1,nwindow
      DO j=1,nbv
        CALL ML_predict(xf(:,j,n),xf_bufr)
        CALL set_h(xf_bufr) !!! incorrect but anyway no half since Hx is linear
!        CALL set_h(xf(:,j,n))
        h4d(:,:,n) = h
        hxf(:,j,n) = h4d(:,1,n) *  xf_bufr(1)
!        hxf(:,j,n) = h4d(:,1,n) * xf(1,j,n)
        DO i=2,nx
          hxf(:,j,n) = hxf(:,j,n) +  h4d(:,i,n) *  xf_bufr(i)
...
!!!! BIAS CORRECTION BY ML
   CALL ML_predict(xm(:,nn),xm(:,nn))
!!!!
